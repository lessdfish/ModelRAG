...
/** Local Okapi BM25 implementation used when Elasticsearch is not enabled. */
@Service
@Profile("!postgres")
public class LocalBm25Search implements Bm25Search {
    ...
    public LocalBm25Search(KnowledgeStore store) {
        this.store = store;
    }
    ...
    public List<ScoredChunk> search(HybridSearchRequest request, int recallSize) {
        ...
        List<String> queryTerms = unique(tokens(request.query()));
        ...
        for (Chunk chunk : chunks) {
            List<String> terms = tokens(chunk.content());
            documentTerms.put(chunk.id(), terms);
            totalLength += terms.size();
            for (String term : new LinkedHashSet<>(terms)) documentFrequency.merge(term, 1, Integer::sum);
        }
        double averageLength = Math.max(1, totalLength / chunks.size());
        int totalDocuments = chunks.size();

        return chunks.stream()
                .map(chunk -> new ScoredChunk(chunk.id(), chunk.content(),
                        bm25(queryTerms, documentTerms.getOrDefault(chunk.id(), List.of()), documentFrequency,
                                totalDocuments, averageLength),
                        "bm25", 0))
    ...
    private double bm25(List<String> queryTerms, List<String> documentTerms, Map<String, Integer> documentFrequency,
            int totalDocuments, double averageLength) {
        if (documentTerms.isEmpty()) return 0;
        Map<String, Integer> termFrequency = new HashMap<>();
        for (String term : documentTerms) termFrequency.merge(term, 1, Integer::sum);
    ...
    private List<String> tokens(String value) {
        ...
        Matcher matcher = LATIN_OR_NUMBER.matcher(text);
        while (matcher.find()) result.add(matcher.group());

        String compact = text.replaceAll("[\\s，。！？、：:；;（）()【】\\[\\]{}《》“”\"'`~!@#$%^&*+=|\\\\/<>,.?-]+", "");
        for (int i = 0; i + 1 < compact.length(); i++) result.add(compact.substring(i, i + 2));
