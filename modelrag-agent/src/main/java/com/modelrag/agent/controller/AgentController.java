...
@RestController
@RequestMapping("/api/v1/qa/agent")
public class AgentController {
    ...
    public AgentController(AgentOrchestrator a, SseEmitterService s, AccessControlService access, ConversationMemory memory, ApprovalGate approvals, AgentExecutionRegistry executions) {
        agent = a;
        sse = s;
        this.access = access;
        this.memory = memory;
    ...
    @PostMapping
    public ApiResponse<AgentResult> execute(@RequestBody QaRequest request) {
        ...
        return ApiResponse.success(agent.execute(new QaRequest(request.datasetId(), request.query(), request.conversationId(), user.id(), user.roles())));
    }
    ...
    public ApiResponse<List<ApprovalSnapshot>> approvals() {
        var user = access.requireRole("APPROVER");
        return ApiResponse.success(approvals.pendingFor(user));
    }

    @PostMapping("/approval/{approvalId}")
    public ApiResponse<AgentResult> approve(@PathVariable String approvalId, @RequestBody QaRequest request, @RequestParam(defaultValue = "true") boolean approved) {
        var user = access.requireRole("APPROVER");
        ApprovalRecord record = approvals.validateDecision(approvalId, user.id(), request.datasetId(), request.conversationId());
        ...
        String requesterId = record.requesterUserId() == null ? request.userId() : record.requesterUserId();
        access.requireDatasetAccess(datasetId);
        return ApiResponse.success(agent.continueAfterApproval(approvalId, new QaRequest(datasetId, request.query(), conversationId, requesterId, user.roles()), approved, user.id()));
    }
}
