...
function Invoke-Upload {
  ...
  $uri = "$BackendUrl/api/v1/knowledge-bases/$DatasetId/documents"
...
$datasetResponse = Invoke-Api Post "/knowledge-bases" @{
  name = $datasetName
...
$QuestionHeaders = Login-User $userId $userPassword
$ApproverHeaders = Login-User $approverId $userPassword
$NoAccessHeaders = Login-User $noAccessId $userPassword

$visibleForUser = (Invoke-Api Get "/knowledge-bases" $null $QuestionHeaders).data
...
$visibleForNoAccess = (Invoke-Api Get "/knowledge-bases" $null $NoAccessHeaders).data
...
$ready = $false
for ($i=0; $i -lt 60; $i++) {
  $docs = (Invoke-Api Get "/knowledge-bases/$($dataset.id)/documents").data
...
$simple = (Invoke-Api Post "/qa/auto" @{ query = $simpleQuery } $QuestionHeaders).data
...
$complex = (Invoke-Api Post "/qa/auto" @{ query = $complexQuery } $QuestionHeaders).data
...
$replay = (Invoke-Api Get "/admin/retrieval-traces/$($simple.traceId)/replay").data
...
$risk = (Invoke-Api Post "/qa/auto" @{ query = $riskQuery } $QuestionHeaders).data
...
$approved = (Invoke-Api Post "/qa/agent/approval/$($risk.approvalId)?approved=true" @{ datasetId = $risk.datasetId; query = $riskQuery } $ApproverHeaders).data
...
Assert-True ($simpleAudit.userId -eq $userId) "RAG audit did not include the requester userId."
Assert-True ($complexAudit.userId -eq $userId) "Agent audit did not include the requester userId."
