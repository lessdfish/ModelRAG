...
@RestController @RequestMapping("/api/v1/admin") public class AdminAuditController {
    ...
    @GetMapping("/qa-audits") public ApiResponse<List<Map<String,Object>>> qaAudits(){access.requireRole("ADMIN");return ApiResponse.success(qa.audits());}
    @GetMapping("/retrieval-traces/{traceId}/replay") public ApiResponse<Map<String,Object>> replay(@PathVariable String traceId){access.requireRole("ADMIN");return ApiResponse.success(qa.replayTrace(traceId));}
    @GetMapping("/approvals") public ApiResponse<List<Map<String,Object>>> approvals(){access.requireRole("ADMIN");return ApiResponse.success(approvals.list());}
